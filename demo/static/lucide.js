/* Lucide 图标（ISC License, https://lucide.dev）—— 内联路径子集，断网可用。
   用法：Lucide.icon("eye",{size:18})  →  <svg …>；或 <i data-lucide="eye"></i> + Lucide.mount()。
   全站禁止表情符号，所有图形符号一律走这里。 */
window.Lucide = (function () {
  var P = {
    ...
    "map-pin-off": '<path d="M12.75 7.09a3 3 0 0 1 2.16 2.16"/><path d="M17.072 17.072c-1.634 2.17-3.527 3.912-4.471 4.727a1 1 0 0 1-1.202 0C9.539 20.193 4 15.093 4 10a8 8 0 0 1 1.432-4.568"/><path d="m2 2 20 20"/><path d="M8.475 2.818A8 8 0 0 1 20 10c0 1.183-.31 2.377-.81 3.533"/><path d="M9.13 9.13a3 3 0 0 0 3.74 3.74"/>',
  ...
  function icon(name, o) {
    o = o || {};
    var s = o.size || 20, sw = o.stroke || 2, cls = o.cls ? " " + o.cls : "";
    var body = P[name];
    if (!body) body = P["circle-dot"];
    return '<svg class="lu' + cls + '" width="' + s + '" height="' + s + '" viewBox="0 0 24 24" fill="none" ' +
  ...
  function mount(root) {
    ...
    for (var i = 0; i < els.length; i++) {
      var el = els[i];
      el.innerHTML = icon(el.getAttribute("data-lucide"),
        { size: +el.getAttribute("data-size") || 20, stroke: +el.getAttribute("data-stroke") || 2 });
    }
  }
  return { icon: icon, mount: mount, has: function (n) { return !!P[n]; } };
})();
